...
- **Redis is rebuildable.** `db.md` §6 treats Redis as rebuildable; any
  non-rebuildable use needs its own persistence policy. The worker's job host
  runs only work that is safe to miss, and durable one-shot jobs wait for that
  policy (`apps/worker/AGENTS.md`).
...
- **The worker cannot reach the runtime.** The runtime lives in `apps/api`,
  which the worker may not import.
...
- **Accept.** In one Postgres transaction the API claims the conversation's
  ...
  - The turn row carries what the worker and the reconciler need and the
    ...
    session id, and the budget hold (company and global reservation, Kyiv
    date).
  - **An answer keeps its synchronous half.** The pause is claimed and the
    ...
    (SHO-546). The action's result and card go into the job as the parts
    already earned. Order: claim the pause, run the action, then accept; an
...
- **Execute.** A BullMQ `assistant` queue on the worker runs the turn as the
  ...
  worker refuses a job whose session row is gone or expired (a direct read of
  the `session` table; the worker has no better-auth). A server-side timeout
...
- **Fail visibly, never twice.** A turn runs once (`attempts: 1`), and a job
  ...
  - Deploys drain in-flight turns; the stop grace period is at least the turn
    timeout.
- **Persist the queue.** The Redis that holds BullMQ runs with AOF
  (`appendonly yes`, `appendfsync everysec`) on a persistent volume. That makes
  it this system's first non-rebuildable Redis use, so the policy lands in
  `db.md` §6. Up to a second of queue loss is covered by the reconciler,
  because the accepted turn is already in Postgres. There is no production
  environment yet: the compose file is the development form of this policy,
  and the production form is a requirement recorded now and checked when the
  infrastructure is built.
  - The job payload carries the client IP for the audit trail of the turn's
    actions, and AOF puts it on disk. It is transient — jobs are removed on
    completion and on failure — and this is the reason the retention is not
    a knob.
...
- **Amendments and specs.** ADR-0038 is amended: the turn lease and command
  ...
  completes rather than at the end of the turn. `db.md` §6 gains the queue's
  persistence policy, and `apps/worker/AGENTS.md` allows durable assistant
  jobs under it.
...
- **The API becomes a producer.** It gains BullMQ as a dependency, for the
  `Queue` only; it never processes jobs.
