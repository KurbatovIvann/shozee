...
| `search` | Staff tenant-private `search.query` orchestrator (zero tables, zero projections, zero read-model grants; pure `ctx.call` fan-out — SHO-526) **and** global FTS/trigram discovery projections for published companies/products and public counters | Staff path: `ctx.call` of owner matchers (`customers.searchMatches`, `catalog.searchMatches`, `orders.searchMatches`, `pricing.searchMatches`, `documents.searchMatches`) plus `companies.get` for numbering prefix. Public/consumer discovery (ADR-0020) stays a later feature on the same module under a second principal; that path consumes events or declared read-model grants from `companies`/`catalog`/`orders`; owns no domain authority or pricing data |
| `assistant` | AI conversation identity, its message log, provider history, and its turns | Four tables: the conversation; the message log, one row per message (ADR-0038, SHO-555); one row of provider `history` per conversation; and one row per accepted turn — the conversation's lease and the command's receipt (ADR-0039, SHO-560). Message payloads and history are opaque to the module — their shapes belong to the runtime that writes them. What the assistant *did* is in `audit_log` under `channel = 'ai'`, not here. Never duplicates domain state as a projection or source of truth |
