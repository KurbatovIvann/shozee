...
| `assistant` | AI conversation identity and its stored chat state | Two tables: the conversation, and one row of `document` / `history` per conversation (ADR-0038). Both blobs are replaced whole and are opaque to the module — their shapes belong to the runtime that writes them. What the assistant *did* is in `audit_log` under `channel = 'ai'`, not here. Never duplicates domain state as a projection or source of truth |
