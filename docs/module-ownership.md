...
| `assistant` | AI conversation/tool-run persistence | Stores action/tool IDs and results; never duplicates domain state as a projection or source of truth. `assistant_tool_runs.model_trace` is ADR-0034 prompt state (bounded post-clip façade output; no client renders it). `tool_input` is bounded façade/tool args for the same prompt-state row (null on pre-T2 rows). `execution_id` is the server-minted attempt identity, unique per tenant, minted before execute. `tool_name` is the live ToolSet key used to reconstruct that trace (`orders_list_page`), not a second domain API |
