...
/**
 * Collect façade / provider tool calls from UI-message SSE. Skips a
 * leftover `json` tool name if a model still emits one. Overlay `execute`
 * results by `toolCallId` when the provider does not stream them
 * (`tool_search`).
 */
