...
export const readLatestInterruptedTurnContract = defineActionContract({
  ...
  description: `${STAFF_CONVERSATION_AUTHOR_INVARIANT} Read the commandId of the most recently interrupted turn of this conversation — Продовжити resolves which turn to continue from this, never from a client-supplied id. Its endReason is the stored cause (not_started, job_exhausted or timeout), null for an ordinary interruption. Both are null when no turn of this conversation is interrupted. A conversation belonging to another author or another company is not-found. Company id is never input.`,
  principal: "staff",
  transport: "internal",
