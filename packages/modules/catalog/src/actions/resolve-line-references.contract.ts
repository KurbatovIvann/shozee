...
export const resolveLineReferencesContract = defineActionContract({
  ...
  description:
    "Resolve a batch of order lines in the staff member's active company from product and optional variant ids or unique names. variantSelection is unspecified, base, or a reference (id or query). Legacy variant EntityRef is mutually exclusive with variantSelection and maps to reference. Product and variant ids must be active. Product query matches include active and archived rows: a unique active match wins even when an archived row has a similar name; archived-only matches return a structured catalog conflict (reason archived, empty options, optionsTruncated false) that names a unique archived product or keeps the query as subject when several match, and never sells; zero matches in both are not-found. A product with any variant rows is variable: unspecified or base requires exactly one active variant and never sells the parent; archived-only variants are unavailable. Zero variant rows may resolve variantId null. Variant queries are scoped to the resolved product and match active rows only. Zero product matches are not-found, with one exception: a product query with no variant selection that matches no product is read as product words followed or preceded by variant words, and resolves only when exactly one such reading gives a unique active product and a unique active variant of it. A variant id or query on a product with zero variant rows is not-found. A unique exact name resolves, and so does a unique product or variant whose whole name is the query in another Ukrainian case (same words). Ambiguous, partial, substring or typo product queries among active rows return a structured catalog conflict (reason, server-side line target, options, optionsTruncated) and never auto-select; typo candidates are offered only when nothing else matches. Variant selection conflicts use the same structured conflict. Every line is classified before a throw: the earliest terminal failure by input lineIndex (archived, no_active_variants, or not-found) is returned before any picker (variant_required, ambiguous, unmatched_query). Unexpected infrastructure, invariant, or authorization errors propagate unchanged. Output preserves input order. Company id is never input.",
  principal: "staff",
  transport: "internal",
