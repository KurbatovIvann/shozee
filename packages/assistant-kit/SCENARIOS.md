...
| #   | Level | Given / When / Then                                                                                                                                                                                                                                                          | Prevents                                                                           |
| --- | ----- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------- |
| 16  | K + L | A turn's parts are appended live. → `messages.read` afterwards returns the **same parts in the same order**. Live and reload are the same bytes, not two derivations. Also asserted against a real turn's own `parts`.                                                       | Live and reload composing different cards                                          |
...
| 20  | K + R | A pause opened for one owner. → a claim under another `bind` answers exactly as an absent pause does (410), the owner's card stays open, and nothing lands in the other tenant's conversation. | Cross-tenant resume; late results after a switch (SHO-540)           |
...
| 34  | Two identical sends → the write runs once and the retry is answered with the conversation. Also while the first is still in flight, and for a retried answer, which the exactly-once claim would otherwise refuse with `410` and no window (SHO-547).                               |
| 34b | Two turns on one conversation → the second is refused `turn_open` with the current window, and the first's message survives whole. Also for a chat arriving while an answer is still resolving, which the pause check cannot catch because the claim already consumed it (SHO-548). |
...
`messages.read` scoped only the _pause_ by owner; the messages came back to
...
rather than appended. `messages.write` takes a scope, like the read. The token
